...
├── MediaDeck.ViewModels/       # ViewModels
├── MediaDeck.Core/             # ビジネスロジック・モデル・状態管理
├── MediaDeck.FileTypes/        # ファイルタイプ別の処理実装 (画像、動画、PDFなど)
├── MediaDeck.Database/         # データベース層 (EF Core, SQLite)
├── MediaDeck.Composition/      # 共通インターフェース・コア用DI定義
├── MediaDeck.Composition.UI/   # UI用共通コンポーネント・DI定義
├── MediaDeck.Common/           # 共通ユーティリティ類
└── lib/                        # サブモジュール
