...
public class TagManagerViewModel : ViewModelBase {
	public TagManagerViewModel(TagsManager tagsManager) {
		this._tagCategories = [.. tagsManager.TagCategories.Select(x => new TagCategoryViewModel(new TagCategoryModel(x), tagsManager, x.Tags.Select(t => new TagModel(t))))];
		...
		this.SaveCommand.Subscribe(async _ => {
			...
			this._tagCategories.AddRange(tagsManager.TagCategories.Select(x => new TagCategoryViewModel(new TagCategoryModel(x), tagsManager, x.Tags.Select(t => new TagModel(t)))));
		});
		this.AddTagCategoryCommand.Subscribe(_ => {
			this._tagCategories.Add(new(new TagCategoryModel() {
				TagCategoryName = "",
				Detail = ""
			}, tagsManager));
		});
