...
public class DetailSelectorModelTests {
	...
	/// <summary>
	/// AddTagByNameAsyncが、既存のタグ名が指定された場合にそのタグを追加することを検証します。
	/// </summary>
	[Fact]
	public async Task AddTagByNameAsync_ShouldAddExistingTag() {
		// Arrange
		var dbOptions = new DbContextOptionsBuilder<MediaDeckDbContext>()
			.UseInMemoryDatabase("AddTagByNameTest_Existing")
		...
		using (var db = new MediaDeckDbContext(dbOptions)) {
			db.Database.EnsureDeleted();
			db.Database.EnsureCreated();
			db.TagCategories.Add(new TagCategory { TagCategoryId = 1, TagCategoryName = "Cat1", Detail = "", Tags = [] });
			db.Tags.Add(new Tag { TagId = 1, TagCategoryId = 1, TagName = "ExistingTag", Detail = "", TagCategory = null! });
			db.MediaFiles.Add(new MediaFile { MediaFileId = 100, FilePath = "test", DirectoryPath = "dir", Description = "" });
			await db.SaveChangesAsync();
		}

		var tagsManager = new TagsManager(dbFactoryMock.Object, tagModelFactoryMock.Object);
		var tagMock = new Mock<ITagModel>();
		...
		var fileMock = new Mock<IFileModel>();
		fileMock.SetupGet(x => x.Id).Returns(100);
		var tagsList1 = new List<ITagModel>();
		fileMock.SetupGet(x => x.Tags).Returns(tagsList1);

		// Act
		await model.AddTagByNameAsync([fileMock.Object], "ExistingTag");

		// Assert
		fileMock.Object.Tags.ShouldContain(tagMock.Object);
	}

	/// <summary>
	/// AddTagByNameAsyncが、存在しないタグ名が指定された場合に新規作成して追加することを検証します。
	/// </summary>
	[Fact]
	public async Task AddTagByNameAsync_ShouldCreateAndAddTag() {
		// Arrange
		var tagModelFactoryMock = new Mock<ITagModelFactory>();

		// CreateInMemoryDbFactory を使って Seed した状態にする
		var dbOptions = new DbContextOptionsBuilder<MediaDeckDbContext>()
			.UseInMemoryDatabase("AddTagByNameTest_New")
			.ConfigureWarnings(x => x.Ignore(Microsoft.EntityFrameworkCore.Diagnostics.InMemoryEventId.TransactionIgnoredWarning))
			.Options;
		var dbFactoryMock = new Mock<IDbContextFactory<MediaDeckDbContext>>();
		dbFactoryMock.Setup(x => x.CreateDbContextAsync(It.IsAny<CancellationToken>())).ReturnsAsync(() => new MediaDeckDbContext(dbOptions));
		...
		var realManager = new TagsManager(dbFactoryMock.Object, tagModelFactoryMock.Object);
		using var modelWithRealManager = new DetailSelectorModel(realManager);

		var fileMock = new Mock<IFileModel>();
		fileMock.SetupGet(x => x.Id).Returns(100);
		var tagsList2 = new List<ITagModel>();
		fileMock.SetupGet(x => x.Tags).Returns(tagsList2);

		// Act
		await modelWithRealManager.AddTagByNameAsync([fileMock.Object], "NewTag");

		// Assert
		fileMock.Object.Tags.ShouldNotBeEmpty();
		fileMock.Object.Tags.First()!.TagName.ShouldBe("NewTag");
	}
}